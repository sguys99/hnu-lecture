...
### 4-1. 해시 라우터
- [ ] `hashchange` + 초기 로드 시 라우팅 처리: `#/`(또는 해시 없음)→메인, `#/q/{id}`→상세
- [ ] 존재하지 않는 `id`는 메인 뷰로 폴백(또는 안내)

### 4-2. 상세 뷰 렌더링
- [ ] 질문 메타(Part·번호) + 질문 전문 + 답변 영역 렌더링
- [ ] "← 목록으로" 버튼 + 브라우저 뒤로가기 모두 지원

### 4-3. 딥링크 검증
- [ ] `#/q/3` 직접 진입·새로고침 시 상세 뷰 복원 확인
...
### 5-1. 미니 파서 구현
- [ ] `app.js`에 MD→HTML 변환 함수 구현: 불릿(`- `)·번호 목록(`1. `)·표(`| ... |`)·강조(`**`)·줄바꿈 처리
- [ ] XSS 방지를 위한 기본 이스케이프 처리(입력은 신뢰된 정적 데이터지만 안전 마진 확보)

### 5-2. 상세 뷰 연결
- [ ] `answer` 마크다운 → 파서 → 상세 뷰 답변 HTML 주입

### 5-3. 파서 검증(스크립트성 점검)
- [ ] 불릿/번호/표/강조 혼합 샘플로 변환 결과 수동 점검(브라우저 + 콘솔)
